...
@section('title', $event->nama_event . ' - Detail Event (Admin)')

@section('content')
<div class="mb-4">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('admin.event.index') }}" class="text-decoration-none">Data Event</a></li>
        ...
        <div class="card border-0 shadow-sm rounded-4 mb-4 sticky-top" style="top: 2rem;">
            <div class="card-body p-4">
                ...
                @if($event->status == 2 || \Carbon\Carbon::parse($event->tanggal_event)->isPast())
                    ...
                    <a href="{{ route('admin.klasemen.index', ['event_id' => $event->id]) }}" class="btn btn-outline-secondary w-100 rounded-pill mt-3 fw-bold py-2">
                        <i class="bi bi-trophy me-2"></i> Lihat Hasil Akhir
                    </a>
                @elseif($event->status == 0)
                    <div class="alert alert-warning border-0 rounded-4 text-center py-3 mb-0">
                        <i class="bi bi-hourglass-split me-2"></i> Menunggu persetujuan Anda
                    </div>
                @elseif($event->status == 3)
                    <div class="alert alert-danger border-0 rounded-4 text-center py-3 mb-0">
                        <i class="bi bi-x-circle me-2"></i> Anda telah menolak event ini
                    </div>
                @else
